import { Controller, Get, Post as HttpPost, Body, Param, Delete, UseGuards, Patch, Query } from '@nestjs/common';
...
export class PostsController {
    ...
    @ApiOperation({
        ...
        description: 'Retorna una lista paginada de publicaciones ordenadas por fecha de creación (más recientes primero)'
    })
    @ApiQuery({ name: 'page', required: false, description: 'Número de página', example: 1 })
    @ApiQuery({ name: 'limit', required: false, description: 'Cantidad de publicaciones por página', example: 10 })
    @ApiResponse({
        status: 200,
    ...
    @ApiOperation({
        ...
        description: 'Agrega o quita el like del usuario autenticado a una publicación. Si ya tiene like, lo quita. Si tiene dislike, lo quita y agrega like.'
    })
    @ApiParam({ name: 'id', description: 'ID de la publicación' })
    @ApiResponse({ status: 200, description: 'Like agregado/quitado exitosamente' })
    ...
    @ApiOperation({
        ...
        description: 'Agrega o quita el dislike del usuario autenticado a una publicación. Si ya tiene dislike, lo quita. Si tiene like, lo quita y agrega dislike.'
    })
    @ApiParam({ name: 'id', description: 'ID de la publicación' })
    @ApiResponse({ status: 200, description: 'Dislike agregado/quitado exitosamente' })
    @ApiResponse({ status: 401, description: 'No autorizado - Token JWT requerido' })
    @ApiResponse({ status: 404, description: 'Publicación no encontrada' })
    ...
    @ApiOperation({
        ...
        description: 'Actualiza una publicación existente. Solo el autor puede editar su publicación.'
    })
    @ApiParam({ name: 'id', description: 'ID de la publicación' })
    @ApiResponse({ status: 200, description: 'Publicación actualizada exitosamente' })
    @ApiResponse({ status: 401, description: 'No autorizado - Token JWT requerido' })
    @ApiResponse({ status: 403, description: 'Prohibido - Solo el autor puede editar' })
    @ApiResponse({ status: 404, description: 'Publicación no encontrada' })
    update(
        @Param('id') id: string,
    ...
    @ApiOperation({
        ...
        description: 'Elimina una publicación existente. Solo el autor puede eliminar su publicación.'
    })
    @ApiParam({ name: 'id', description: 'ID de la publicación' })
    @ApiResponse({ status: 200, description: 'Publicación eliminada exitosamente' })
    @ApiResponse({ status: 401, description: 'No autorizado - Token JWT requerido' })
    @ApiResponse({ status: 403, description: 'Prohibido - Solo el autor puede eliminar' })
    @ApiResponse({ status: 404, description: 'Publicación no encontrada' })
    remove(
        @Param('id') id: string,
        @GetUser() user: any
